...
import { AdoptionRequestCard } from "~/components/Animals/AdoptionRequestCard";

export const meta = () => {
	return [
		{ title: "Adoption Requests | ShelterOS" },
		{ name: "description", content: "Review and manage adoption applications." },
	];
};
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
	...
	const data = await animalsApi.getAdoptionsRequests({
		search: q,
		pageIndex,
		pageSize,
	});
...
export default function AdoptionRequestsPage() {
	...
	const adoptionsData = loaderData.success ? loaderData.data : null;
	const adoptions = adoptionsData?.adoptions ?? [];
	const pagination = adoptionsData?.pagination;
			...
			<div className="flex h-[70vh] items-center justify-center p-6">
				<div className="max-w-md w-full text-center">
					...
					<h2 className="text-2xl font-semibold tracking-tight mb-2">
						Failed to load adoptions requests
					</h2>
					<p className="text-muted-foreground mb-6">
						{error?.message || "Something went wrong while retrieving adoption requests."}
					</p>
					<div className="flex flex-col sm:flex-row gap-3 justify-center">
						<Button onClick={() => invalidateCache("all_adoptions")} variant="default">
	...
	return (
		<div className="flex-1 flex flex-col gap-6 p-6 max-w-7xl mx-auto w-full">
			...
			<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
				<div>
					...
					<p className="text-muted-foreground mt-1 text-sm">
						Review applications from adoption volunteers and manage placements.
					</p>
				</div>
			</div>
			...
			<div className="flex flex-col sm:flex-row items-center justify-between gap-4">
				<Form method="get" className="w-full sm:max-w-md">
					<div className="relative">
						...
						<Input
							placeholder="Search by adopter's name, email, animal..."
							name="q"
				...
				<div className="text-xs text-muted-foreground self-end sm:self-center">
					Showing <span className="font-semibold text-foreground">{adoptions.length}</span> of{" "}
			...
			{/* Grid */}
			{isFetching ? (
				<AdoptionsGridSkeleton />
			) : adoptions.length === 0 ? (
				<EmptyAdoptionsState query={currentQuery} />
			) : (
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
					{adoptions.map((a) => (
						<AdoptionRequestCard key={a.id} adoption={a} />
					))}
				</div>
			)}
...
function AdoptionsGridSkeleton() {
	return (
		<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
...
function EmptyAdoptionsState({ query }: { query?: string }) {
	return (
		<div className="flex flex-col items-center justify-center p-12 text-center bg-card rounded-2xl border border-dashed border-border/80 my-4">
			...
			{query && (
				<Button variant="outline" size="sm" className="mt-4" asChild>
					<Link to="/adoptions">Clear Search Filter</Link>
				</Button>
			)}
		</div>
	);
