			...
			{animals.length > 0 ? (
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
					{animals.map((animal) => {
						...
						const personalityTags = animal.personality
							? animal.personality.split(",").map((p) => p.trim())
							: [];

						return (
							<Card
								key={animal.id}
								className="overflow-hidden group flex flex-col justify-between border-border hover:shadow-md transition-all duration-300"
							>
								<div>
				...
				<div className="flex flex-col items-center justify-center p-12 text-center bg-card rounded-xl border border-dashed border-border my-8 space-y-3">
					...
					<p className="text-sm text-muted-foreground max-w-sm">
						We couldn't find any animals matching your search or species selection. Try adjusting
						your search filters.
					</p>
